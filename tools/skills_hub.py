...
def parallel_search_sources(
...
) -> Tuple[List[SkillMeta], Dict[str, int]]:
    """Search all sources in parallel with per-source timeout.

    Returns (all_results, source_counts).  *on_source_done* is an optional
    callback ``(source_id, count) -> None`` invoked as each source completes —
    ...
    with ThreadPoolExecutor(max_workers=min(len(active), 8)) as pool:
        ...
        try:
            ...
            # Some sources didn't finish — that's fine, we return what we have
            timed_out = [
...
def unified_search(query: str, sources: List[SkillSource],
    ...
    all_results, _ = parallel_search_sources(
        sources,
        query=query,
